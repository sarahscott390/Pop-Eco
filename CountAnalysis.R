...
#isn't sensitive to the value you provided. Note: The higher it is, the longer it will take the model to run.
...
#
### 1. Based on the overlap of the 95% CIs for your predictor coefficients, #
# which may be important to each of your responses? #
# Answer: 
# Abundance: Sagebrush's CI does not overlap zero so it is likely important, while cheatgrass does overlap zero and is likely not important.
# Detection: All observer CIs overlap zero, so it seems that they are not important, while Time's CI does not overlap zero and is important.
#
# Time^2 explanation from online references/chat for general understanding: The CI for time (0.732 to 1.788) does not overlap zero, indicating a significant positive effect on detection probability as time increases. Conversely, the CI for time² (-1.986 to -0.887) is negative and does not overlap zero, suggesting a quadratic relationship where detection increases with time to a point but may decline afterward.
#
#What is the mean abundance from our model?  12.06
...
#view
gof.boot
### 2. What does the output tell us about our model fit?
# Answer:  Chi-square = 117.2983.  P value = .86 (seems fine?)... no evidence there that the model is a poor fit.  C-hat = .83, indicating some underdispersion (c-hat < 1 = underdispersed, c-hat > 1 = overdispersed).  Seems like overall, the model fits okay.
#
#
#
...
# Answer:  It tells us that the full model fits the data better than the null model.  AIC is lower and speudo-Rsquared is high.  Habitat and survey predictors are important to abundance and detection.  
#
...
residcov( fm.closed )
### 4. What do the plots tell you?
# Answer:  marginal c-hat = 0.83, site-sum c-hat = 0.88, and observation c-hat = 0.87. All these values are below 1, indicating a bit of underdispersion in the model.   Residual plot looks solid; residual data points are more or less evenly scattered with no curves or major clustering, outliers are not dramatic.
#
...
### 5. What did Knape et al. 2018 say these residuals were useful for?
# Answer: (pg. 2105) Site-sum residuals: "The idea of aggregating counts across sites is to make residuals independent and potentially to increase their informativeness in cases where counts are sparse. Since there is one site-sum residual per site, they can be used in plots against site-specific covariates."  Observation residuals: " The idea is to compute residuals from the observation model only by conditioning on the abundances, with the intent of more specifically checking the detection part of the model."
#
...
### 6. What do these plots indicate? 
# Answer:  Both site sum and observation residuals stay near or on the line (no significant departures from the line), it seems like they are pretty normally distributed/the model fit is solid
# 
### 7. What is some of the advice recommended by Knape et al. 2018 if we want 
# to use abundance estimates from these N-mixture models?
# Answer:  To avoid overestimates of abundance when using N-mixture models, Knape et al suggest using negative binomial, zero inflated Poisson or adding random effects.
#
### 8. Now try fitting other functional forms (e.g. ZIP or Negative Binomial)
# Do you get a better fit?
# Answer: The negative binomial (below) has a slightly higher AIC than the previous model (651 vs 649) so it is not performing better than the N-mixture model we previously fit.  
#
# # Negative Binomial model
fm.nb <- pcount(~ 1 + obsv + time + time2 ~ 1 + sagebrush + cheatgrass, 
                mixture = "NB", 
                K = 40, 
                data = umf)
...
#
### 9. Are there other ways to fit time that may make more sense?
# Answer:  We could treat time as a factor/categorical variable?
#
