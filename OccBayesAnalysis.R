...
#Answer: p is detection across 3 visits (p1, p2, p3)
## two years of data but still using single season model, why?  for multi-season, we would need at least 5 levels.  cant use random intercepts for less than that.
...
#JC: prednames <- c( "crop", "aquatic", "grass", "forest", "shrub", "ah", "lo" ) 

prednames <- c( "crop", "aquatic", "grass", "forest", "barren", "shrub", "water", "ah", "lo") 
...
## Answer:  Aquatic + shrub (-.50) and barren + ah (-.51) are negatively correlated, so it may be best to pick one or the other from each of these pairs based on which have the best data available.  Crop + forest (.44) and lo + shrub are moderately correlated as well, but not too concerning?
...
## Answer: crop and lo are not good, they are really spaced out and left skewed with big gaps.  Not strong enough data to include in the analysis.  Forest and water are also really left skewed but the gaps are not as prominent.. the cautious thing would probably be to exclude these as well.
...
#In class notes:
## they don't need to be normally distributed, but need good variability in the range and not have dramatic outliers
## forest is spotty, may struggle in the model
## crop looks bad, not enough data
## others look good
...
#from these habitat predictors, ### choose the ones you want to include ### different than ones selected below, make better choices from graphs from 2012 ###
# We extract those habitats into a new dataframe
X <- habdf[,c("aquatic", "grass", "barren", "shrub", "ah") ]
...
#Answer:
### wind looks good, cloud cover doesn't look great.  Not sure if its bad enough that it shouldnt be used?  High at each end, low in the middle.  I think I'll play it safe for now and use wind.

#we create a siteXsurvey matrix for each one and standardise them
#wind
wind_sc <- scale( detdf[ ,c("wind1", "wind2", "wind3") ])
#replace missing values with mean 0, so it can still be used in the model 
###(since this value is scaled, the mean value is zero)###
...
nt <- 10; nb <- 20000; nc <- 5 #thinning, burning, chains
