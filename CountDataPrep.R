...
### 1. Why do we use right_join instead of left_join or other?
# Answer: We want to retain all records from preddf in the new dataset.  If we used left join, we would have potentially lost   predictor data for sites that weren't surveyed.
#
# opendf contains information about sites where point counts were conducted.
# preddf contains predictor data for all sites and years.
#
...
# X-axis: maximum count of Piute ground squirrels observed at each site during the primary season for each year. This value is calculated as the highest count from the three replicate surveys for each site in a given year.
# Y-axis: frequency (or count) of sites that fall into each bin of the maximum count. Essentially, it shows how # many sites had a particular maximum count of squirrels.
#
...
### 2. What do we see? What is happening to site abundances, but also to #
# site occupancy? 
# Answers: There seem to have been fewer high maximum counts (ex. less counts of ~20) in certain years; 2010, 2011, 2012, 2013, 2014, 2016, 2017, and 2018.  
# Counts seem somewhat consistent overall though, based on the second graphic.  Some sites saw a jump and a few trend down but not steeply.  This takeaway seems inconsistent with our discussions thus far so I'm unsure if I am interpreting things correctly.
...
closeddf <- opendf %>% 
  #filter only rows for desired year *choose a different year than JC*:
