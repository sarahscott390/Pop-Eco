...
# "aquatic", "grass", "barren", "shrub", "ah"

#column labels of predictors used in the model
psilabs <- c("aquatic", "grass", "barren", "shrub", "ah")
...
Wind_Detect <- ggplot( data = p.pred, aes( x = Wind, y = Mean ) ) +
  theme_classic() +
  ggtitle( "Great Horned Owl: Effects on Detection Probability" ) + # change this for different owl species
  ylab( "Probability of Detection" ) +
  xlab( "Wind speed (miles/hr)" ) +
...
ggsave("Wind_Detection_Plot.png", plot = Wind_Detect, width = 8, height = 6, dpi = 300)


GHO_Occ_Plot <- ggplot( data = occ.preds, aes( x = Raw, y = Mean, 
                               fill = as.factor(Year) ) ) +
  theme_classic() +
  ggtitle( "Great Horned owls: Habitat Effects on Occupancy" ) + 
  ylab( "Probability of Occupancy" ) +
...
ggsave("GHO_Occupancy_Plot.png", plot = GHO_Occ_Plot, width = 8, height = 6, dpi = 300)
