...
#### Now replot these for the other model that we run. How do they differ?
# Answer:  The results look very similar for Herbaceous Cover x Ground squirrel abundance and for Detection Probability x weather variables.  This lack of visible variation could be due to the fact that the models share predictors and response variables.  The effect of the predictors in these models could be homogenous/differences between the two are perhaps better visualized with other types of output.  Alternatively, and perhaps more likely, I am missing something!  
# 
lam.preds.m1 <- estppreds( type = 'abundance',
                        sl = 100, 
                        int = m1$sims.list$int.lam, 
                        coefs = m1$sims.list$beta,
                        rawpreds = ik_df, 
                        labs = "herbaceous",
                        nicelabs = c("Herbaceous cover (%)")
)

ggplot( data = lam.preds.m1, aes( x = Raw, y = Mean ) ) +
  theme_classic() +
  ylab( "Ground squirrel abundance" ) +
  xlab( "" ) +
  theme( legend.position = "top", 
...
p.preds.m1 <- estppreds( type = 'detection',
                      sl = 100, 
                      int = m1$sims.list$int.det, 
                      coefs = m1$sims.list$alpha,
                      rawpreds = ikj_df, 
                      labs = c("wind_kmph_st", "tempC_st", "effort_mins" ), 
                      nicelabs = c( "Wind (km/hr)", "Mean temperature (C)",
                                    "Effort (mins)" )
...
ggplot( data = p.preds.m1, aes( x = Raw, y = Mean ) ) +
  theme_classic() +
  ylab( "Detection probability" ) +
  xlab( "" ) +
  theme( legend.position = "top", 
