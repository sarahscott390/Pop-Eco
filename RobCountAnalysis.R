...
# Answer: Both predictors have negative confidence intervals, suggesting a consistent trend towards lower detection probabilities.  
#
...
# Answer: As percentage sage increases, recruitment increases in the ground squirrel population.  I think this makes sense given what we have seen so far with the positive effects of sagebrush on ground squirrel occupancy.
#
...
opendf$avg_time <- rowMeans(opendf[, c("time.j1", "time.j2", "time.j3")], na.rm = TRUE)

time <- seq(min(opendf[,"avg_time"]), max(opendf[,"avg_time"]), length.out = n)
time.std <- scale(time)

timeData <- data.frame(time = time.std)

pred.time <- predict(fm, type = "det", newdata = timeData, appendData = TRUE)

timeplot <- cbind(pred.time[, c("Predicted", "lower", "upper")], time)

timep <- ggplot(timeplot, aes(x = time, y = Predicted)) +
  theme_bw(base_size = 15) +
  labs(x = "Average Detection Time", y = "Probability of Detection") +
  geom_ribbon(aes(ymin = lower, ymax = upper), alpha = 0.5, fill = "grey") + 
  geom_line(size = 2)
